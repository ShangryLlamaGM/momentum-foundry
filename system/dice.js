...
const highestValue = Math.max(...keptNumbers); // Find the highest value
const highestCount = keptNumbers.filter((n) => n === highestValue).length; // Count its occurrences
const flare = highestCount > 1; // Trigger a Flare only if the highest value is duplicated


  let displayFormula = `${totalDice}d`
  if (dicePool.cut > 0)
    displayFormula += ` ${game.i18n.localize('empyrean.cut')} ${dicePool.cut}`
