...
export default class EmpyreanActorSheet extends ActorSheet {
  async getData() {
    const context = super.getData()
    context.config = EMPYREAN
    context.showBurnTooltip = game.settings.get('empyrean', 'showBurnTooltip')
    context.showAttributeTooltip = game.settings.get('empyrean', 'showAttributeTooltip')
    return context
  }
  ...
  itemContextMenu = [
    {
      name: game.i18n.localize('empyrean.toChat'),
    ...
    {
      name: game.i18n.localize('empyrean.edit'),
    ...
    {
      name: game.i18n.localize('empyrean.delete'),
  ...
  async addSlimItem(itemType, itemSubtype = null) {
    const data = await renderDialog(
      game.i18n.format('empyrean.newSlim', {
        type: game.i18n.localize(`empyrean.${itemType}`),
      }),
    ...
    const slimData = {
      id,
      text,
      ...(EMPYREAN.slimDefaults[itemType] || {}),
    }
  ...
  async editSlimItem(itemId, itemType) {
    ...
    if (item) {
      const data = await renderDialog(
        game.i18n.format('empyrean.editSlim', {
          type: game.i18n.localize(`empyrean.${itemType}`),
        }),
        this.processSlimDialog,
        item,
  ...
  async sendSlimToChat(itemId, itemType) {
    ...
    if (item) {
      item.title = game.i18n.localize(`empyrean.${itemType}`)

      const template = 'systems/empyrean/templates/chat/slim.hbs'
  ...
  async sendItemToChat(itemId) {
    ...
    if (item) {
      const template = 'systems/empyrean/templates/chat/item.hbs'
      const html = await renderTemplate(template, item)
