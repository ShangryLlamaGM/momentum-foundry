...
export class EmpyreanTrackDatabase extends Collection {
  getTrackData() {
    return game.settings.get('empyrean', 'activeTracks')
  }
  ...
  async showTrackDialog(title, data = {}) {
    return await renderDialog(
      ...
      { ...data, config: EMPYREAN },
      '/systems/empyrean/templates/applications/tracks/dialog.hbs',
    )
  }
  ...
  addTrack(data = {}) {
    ...
    if (label.trim() === '' || groups === '')
      return ui.notifications.warn(
        game.i18n.localize('empyrean.TRACKS.requiredFields'),
      )
  ...
  moveTrack(id, newIndex) {
    ...
    game.settings.set('empyrean', 'activeTracks', newData)
  }
